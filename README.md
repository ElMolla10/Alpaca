...
flowchart TB
    ...
    Pref[(Risk Preference File<br/>app/data/user_style.json)]
    ...
    PCC -->|POST /api/preferences<br/>bounded UserStyle update| Pref

    Engine -->|Reads active UserStyle each block| Pref
    ...
    classDef pref fill:#fff7ed,color:#111827,stroke:#fb923c,stroke-width:1px;
...
The Project Command Center is **non-executing**. It can inspect broker/account state, persisted records, project metadata, and DriftWatch tables. It never submits broker orders, never modifies positions, and never directly mutates trading state.

Its only write path is a bounded operator preference update: the selected `UserStyle` risk profile is written to `app/data/user_style.json` through `POST /api/preferences`. The trading engine reads this preference at block boundaries and rebuilds the `ARLAgent` from the selected profile, so risk-tolerance changes can take effect without restarting the engine.
...
| Operator risk preference control | Project Command Center → `app/data/user_style.json` → engine block loop |
...
│   │
│   ├── data/
│   │   └── user_style.json          # Persisted operator-selected risk profile
│   │
│   ├── agent/
│   │   ├── __init__.py
...
| Risk Preference | Operator-selected `UserStyle` persisted through `POST /api/preferences` |
...
| Config | Environment, dependency status, and active profile |
| Roadmap | Future-work directions |

### Risk Preference Control

The Project Command Center includes a real operator risk preference control.
...
When the operator changes the profile, the command center writes the selected style to:

```text
app/data/user_style.json
```
...
The active profile is shown in the Config section of the command center.

The trading engine reads the stored preference at each block and rebuilds the `ARLAgent` using the selected `UserStyle`. This means the operator can change risk tolerance without restarting the engine process.

This does **not** give the command center trading authority. The command center does not submit broker orders and does not modify positions. Its write access is limited to this bounded risk-preference file.
...
The active risk profile can be selected from the Project Command Center. The selected `UserStyle` is persisted to `app/data/user_style.json`, and the trading engine reads it at block boundaries before rebuilding the `ARLAgent`.
...
`USER_STYLE` acts as the environment-level default. When `app/data/user_style.json` exists, the trading engine uses the operator-selected profile stored there, allowing the Project Command Center to adjust risk preference without a restart.
...
9. Read the active risk preference from `app/data/user_style.json` when present.
10. Rebuild the ARLAgent from the selected `UserStyle` at block boundaries.
11. Ask the ARLAgent for decisions.
12. Convert decisions to target positions.
13. Apply risk guardrails.
14. Submit paper orders.
15. Record fills.
16. Log DriftWatch inference events.
17. Persist engine state.
...
- Project Command Center `/api/preferences` risk-preference round trip,
...
- state persistence,
- dynamic risk-profile consumption through `current_agent_style()`.
